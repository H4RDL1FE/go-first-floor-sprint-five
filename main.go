...
type Training struct {
	TrainingType string        // тип тренировки (Бег, Ходьба, Плавание)
	Action       int           // количество повторов (шаги, гребки)
...
func (t Training) meanSpeed() float64 {
	// Проверка на 0, думаю имелось ввиду именно это
...
type InfoMessage struct {
	...
	Distance     float64       // расстояние, которое преодолел пользователь, в километрах
...
type Walking struct {
	Training
	Height float64 // рост пользователя в сантиметрах или метрах (в зависимости от того, как вы хотите рассчитывать)
}
...
func (w Walking) Calories() float64 {
	...
	return ((CaloriesWeightMultiplier*w.Weight + (speedInMetersPerSecond*speedInMetersPerSecond/(w.Height/CmInM))*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours)
}
...
func ReadData(training CaloriesCalculator) string {
	// Получаем информацию о тренировке через интерфейс CaloriesCalculator
	info := training.TrainingInfo()

	// Форматируем полученные данные в строку и возвращаем ее
	return fmt.Sprintf("Тип тренировки: %s\nДлительность: %v мин\nДистанция: %.2f км\nСр. скорость: %.2f км/ч\nПотрачено ккал: %.2f",
		info.TrainingType,
